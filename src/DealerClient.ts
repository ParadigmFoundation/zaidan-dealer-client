...
export class DealerClient {
  ...
  public async getQuote(
    ...
    const { status, data } = await this._call("quote", "GET", { size, symbol, side, takerAddress });
    if (status === 200) {
      return data;
    } else if (status === 400) {
  ...
  public async getSwapQuote(
    ...
    const { data, status } = await this._call("swap", "GET", { size, dealerAsset, clientAsset, takerAddress });
    if (status === 200) {
      return data;
    } else if (status === 400) {
  ...
  public async handleTrade(order: SignedOrder, quoteId: string, takerAddress: string = this.coinbase): Promise<string> {
    ...
    try {
      ...
      const { data, status } = await this._call("order", "POST", req);
      if (status === 200) {
        return data.txId;
      } else if (status === 400) {
        throw new Error(data.error);
